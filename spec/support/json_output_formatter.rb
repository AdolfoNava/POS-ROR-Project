...
class JsonOutputFormatter < RSpec::Core::Formatters::JsonFormatter
  ...
  def dump_summary(summary) # rubocop:disable Metrics/AbcSize,Metrics/MethodLength
    total_points = summary.examples.map { |example| example.metadata[:points].to_i }.sum

    earned_points = summary.examples.select do |example|
                      example.execution_result.status == :passed
                    end.map { |example| example.metadata[:points].to_i }.sum # rubocop:disable Style/MultilineBlockChain
